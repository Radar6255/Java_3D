...
public class CombinedCubeBackup extends CubeObject{
...
//	public static float[][] centerVerts = { { 0.0f, 0.0f, 0.5f }, { 0.0f, 0.0f, -0.5f }, { 0.5f, 0.0f, 0.0f },{ -0.5f, 0.0f, 0.0f }, { 0.0f, -0.5f, 0.0f }, { 0.0f, 0.5f, 0.0f } };
	...
	public CombinedCubeBackup(int x, int y, int z, int w, int h, int d, Handler handler, int cubeIndex,int chunkX,int chunkZ, Chunk chunk) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.w = w;
...
//						g.fillOval((int) ((tx * f) + (Main.WIDTH / 2) - 2), (int) ((ty * f) + (Main.HEIGHT) - 2), 4, 4);
						...
						//Distance is now calculated to closest point on face not center of the face
...
//						}
						...
						if (pointsX[0] < px || pointsX[1] < px || pointsX[2] < px || pointsX[3] < px){ //If any point is left of the player
							for(i=0;i < 4;i++){
								if (tx > pointsX[i]+distOff){ //Find right most point in cube
...
//							System.out.println("right?");
						...
						if (pointsZ[0] < pz || pointsZ[1] < pz || pointsZ[2] < pz || pointsZ[3] < pz){
							for(i=0;i < 4;i++){
...
//							System.out.println("Left???");
						...
						if (px > x && x+w > px){
							...
							//System.out.println(tx+" "+ty+" "+tz);
							test2 = true;
						}if (pz > z && z+d > pz && !back && !front && !lside && !rside){
